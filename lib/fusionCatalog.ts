...
import catalogJson from "@/reference/SkyShards-master/public/fusion-data.json";
...
export function selectBestMarketRecipes(
  ...
  const inputPrice = (id: string): number | null => {
    if (!inputPrices.has(id)) {
      inputPrices.set(id, products[id] ? getInputPrice(products[id]) : null);
    }
  ...
  for (const [outputShortId, quantityGroups] of Object.entries(source.recipes)) {
    ...
    for (const [quantityText, inputPairs] of Object.entries(quantityGroups)) {
      ...
      for (const [leftShortId, rightShortId] of inputPairs) {
        ...
        const leftPrice = inputPrice(leftShard.internal_id);
        const rightPrice = inputPrice(rightShard.internal_id);
        const sellPrice = outputPrice(outputShard.internal_id);
        if (leftPrice === null || rightPrice === null || sellPrice === null) continue;
        pricedCandidates += 1;

        const cost =
          leftPrice * leftShard.fuse_amount +
          rightPrice * rightShard.fuse_amount;
        const outputKey = `${outputShortId}:${outputAmount}`;
  ...
  return {
    ...
    stats: {
      ...
      pricedCandidates,
      unavailableCandidates: seen.size - pricedCandidates,
